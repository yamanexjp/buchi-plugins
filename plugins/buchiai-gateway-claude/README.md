# buchiai-gateway — ぶち AI ゲートウェイ接続プラグイン
...
2. **接続中は Remote Control 系機能が動作しません（#230）。**
   ...
   Slack 連携・定期実行は使えません。`/buchiai-gateway:doctor` でも毎回警告します。
...
4. **トークンの保存場所は 2 段階あります（隠さず書きます）。**
   - プラグイン設定（userConfig）に入力した `gateway_token` は、Claude Code に
     ...
     保管されます。
   - ただし接続を有効化（`/buchiai-gateway:setup` を適用）すると、トークンは
...
- Claude Code **2.1.154 以上**（userConfig 対応。`/buchiai-gateway:doctor` の D1 で検査。
...
`settings.json` には書かれません（`/buchiai-gateway:setup` を適用するまでは）。
...
# 2. プラグインをインストール
/plugin install buchiai-gateway@buchi-plugins
...
/buchiai-gateway:setup

# 5. Claude Code を再起動

# 6. 実際に通過しているか確認
/buchiai-gateway:verify
```
...
| コマンド | 種別 | 説明 |
|---|---|---|
| `/buchiai-gateway:setup` | 変更（要同意） | 接続をセットアップ。dry-run で差分を提示し、同意後にのみ `settings.json` へ書込。旧値は退避し、バックアップを直近 5 世代保持 |
| `/buchiai-gateway:off` | 変更（要同意） | 接続を解除し、**setup 前の状態へ完全復元**（元が未設定ならキー削除）。既存の他キーは壊しません |
| `/buchiai-gateway:on` | 変更（要同意） | 解除後の再接続（setup 再実行相当。初回は `/buchiai-gateway:setup`） |
| `/buchiai-gateway:status` | 照会 | 接続先（マスク表示）・反映状態（再起動待ち検知）・healthz 疎通を表示 |
| `/buchiai-gateway:doctor` | 照会 | 診断 D1〜D7（バージョン / JSON 妥当性 / env 競合 / 疎通 / トークン有効性(doctor は送信しない・verify へ誘導) / Remote Control 警告 / コスト効果の正直な説明） |
| `/buchiai-gateway:verify` | 照会（1 リクエスト送信） | **実際にゲートウェイを通過しているか**を無害なテスト要求で確認。「設定済み」「疎通成功」「稼働セッションが設定を使用」「実際の通過（トークン受理→検査→上流転送）」を分けて表示。既定はプレースホルダ鍵でコスト 0（上流が鍵を拒否する通常構成の場合。鍵を無視する上流では出力 1 トークン分）。`full: true` で env の `ANTHROPIC_API_KEY` を使い上流応答まで確認（出力 1 トークン分） |
...
`/buchiai-gateway:status` は「設定済み」「疎通成功」までを、`/buchiai-gateway:verify` はそれに加えて
「稼働セッションの env が設定と一致しているか」「ゲートウェイがトークンを受理して
上流へ転送したか」を、それぞれ独立した OK/NG として表示します。接続作業の
最後は必ず `/buchiai-gateway:verify` で締めてください。
...
1. `/buchiai-gateway:off` を実行（差分確認 → 同意 → setup 前の settings.json へ完全復元）
2. Claude Code を再起動
3. 必要なら `/plugin uninstall buchi`

退避情報が失われている場合、`/buchiai-gateway:off` は `settings.json.buchi-backup-*`
（直近 5 世代）からの手動復元手順を提示します。

## トラブルシューティング

まず `/buchiai-gateway:doctor` を実行してください。DNS / TLS / プロキシ
